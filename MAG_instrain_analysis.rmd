...
library(DescTools) #Has conflict with ecodist package. This should be loaded first so that ecodist can overwrite dependencies. Concordance analysis still works with ecodist dependencies being prioritized.
library(geosphere)
library(ape)
library(vegan)
library(MKinfer)
...
mitocaller_out <- ani_matrix %>% 
  ...
  mutate(instrainANI = 1 - mito_host_conANI_dist) %>% 
  select(pairwise, ANI, instrainANI) %>%
  filter(!is.na(ANI))
